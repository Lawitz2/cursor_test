...
- [x] Подключение к PostgreSQL и применение существующих миграций.
- [x] Генерация базового кода sqlc для таблиц `users`, `products`, `categories`.
- [x] Создание Health-check эндпоинта и базового роутера.

## Этап 2: Каталог товаров (День 3-4)
- [x] **Backend:** Реализация репозиториев для категорий и товаров (List, GetBySlug).
- [x] **API:** Эндпоинты `GET /api/v1/categories` и `GET /api/v1/products` (с базовой фильтрацией по категории).
- [x] **Frontend (SSR):** Базовый шаблон (`base.tmpl`) с использованием простого CSS (например, Pico.css или Bootstrap для скорости).
- [x] **Frontend (SSR):** Страница списка товаров и страница карточки товара.

## Этап 3: Корзина (День 5)
- [x] **Backend:** Логика хранения корзины в БД (используем `guest_token` в Cookie).
- [x] **API:** Добавление, удаление и изменение количества товаров в корзине.
- [x] **Frontend (SSR):** Страница корзины с возможностью изменения состава заказа.

## Этап 4: Упрощенная Авторизация (Заглушка) (День 6)
- [x] **Логика:** Вместо реальной отправки SMS, код подтверждения всегда `1234` (или выводится в лог сервера).
- [x] **API:** Эндпоинты для запроса кода и верификации.
